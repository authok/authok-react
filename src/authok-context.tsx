...
/**
 * Contains the authenticated state and authentication methods provided by the `useAuthok` hook.
 */
export interface AuthokContextInterface<TUser extends User = User>
  ...
  /**
   * ```js
   * authok.logout({ returnTo: window.location.origin });
   * ```
   *
   ...
   * parameters will be auto-generated.
   */
  buildAuthorizeUrl: (options?: AuthokRedirectLoginOptions) => Promise<string>;
 ...
 */
const stub = (): never => {
  throw new Error('You forgot to wrap your component in <AuthokProvider>.');
};
 ...
 * The Authok Context
 */
const AuthokContext = createContext<AuthokContextInterface>(initialContext);

export default AuthokContext;
