...
import AuthokContext from '../src/authok-context';
...
const clientMock = jest.mocked(new AuthokClient({ client_id: '', domain: '' }));

describe('AuthokProvider', () => {
  afterEach(() => {
    window.history.pushState({}, document.title, '/');
  });

  it('should provide the AuthokProvider result', async () => {
    const wrapper = createWrapper();
    const { result, waitForNextUpdate } = renderHook(
      () => useContext(AuthokContext),
      { wrapper }
    );
  ...
  it('should configure an instance of the AuthokClient', async () => {
    const opts = {
      clientId: 'foo',
      domain: 'bar',
      redirectUri: 'baz',
    ...
    const { waitForNextUpdate } = renderHook(() => useContext(AuthokContext), {
      wrapper,
    });
  ...
  it('should pass user agent to AuthokClient', async () => {
    const opts = {
      clientId: 'foo',
      domain: 'bar',
    };
  ...
  it('should skip redirect callback for non authok redirect callback handlers', async () => {
    clientMock.isAuthenticated.mockResolvedValue(true);
    window.history.pushState(
      {},
      document.title,
      '/?code=__some_non_authok_code__&state=__test_state__'
    );
    clientMock.handleRedirectCallback.mockRejectedValue(
      new Error('__test_error__')
    );
  ...
  it('should call getAccessTokenSilently in the scope of the Authok client', async () => {
  ...
  it('should handle not having a user while calling getAccessTokenSilently', async () => {
    ...
    clientMock.getUser.mockResolvedValue(undefined);
    const wrapper = createWrapper();
    const { result } = renderHook(() => useContext(AuthokContext), { wrapper });
    let returnedToken;
    await act(async () => {
  ...
  it('should call getAccessTokenWithPopup in the scope of the Authok client', async () => {
  ...
  it('should call handleRedirectCallback in the scope of the Authok client', async () => {
